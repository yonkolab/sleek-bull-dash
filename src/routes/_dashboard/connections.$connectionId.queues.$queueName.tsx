...
function QueuePage() {
  const { connectionId, queueName } = Route.useParams()
  const [status, setStatus] = useState<QueueStatus>('completed')
  const [page, setPage] = useState(0)
  const [detailJobId, setDetailJobId] = useState<string | null>(null)
  const queryClient = useQueryClient()

  const queryKey = ['queue-jobs', connectionId, queueName, status, page]
  ...
  const retryMutation = useMutation({
    mutationFn: (jobId: string) =>
      $retryJob({ data: { connectionId, queueName, jobId } }),
    onSuccess: () => {
      toast.success('Job queued for retry')
      queryClient.invalidateQueries({ queryKey: ['queue-jobs', connectionId, queueName] })
    },
  ...
  const removeMutation = useMutation({
    mutationFn: (jobId: string) =>
      $removeJob({ data: { connectionId, queueName, jobId } }),
    onSuccess: () => {
  ...
  const cleanMutation = useMutation({
    mutationFn: () =>
      $cleanQueue({ data: { connectionId, queueName, status: status as 'completed' | 'failed' } }),
    onSuccess: () => {
    ...
    <div className="flex flex-col h-full">
            ...
            {isLoading ? (
              ...
              jobs.map((job: JobSummary) => (
                <JobRow
                  ...
                  onViewDetail={() => setDetailJobId(job.id)}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-6 py-3 border-t border-border text-xs text-muted-foreground">
          <span>
            Showing {page * PAGE_SIZE + 1}–{Math.min((page + 1) * PAGE_SIZE, currentCount)} of{' '}
            {currentCount}
          </span>
...
function JobRow({ job, status, onRetry, onRemove, onViewDetail, retrying, removing }: JobRowProps) {
  return (
    <TableRow className="text-xs group cursor-pointer" onClick={onViewDetail}>
      {/* Name */}
      <TableCell className="py-2.5">
        <div className="flex flex-col gap-0.5">
          ...
          {job.failedReason && (
            <span
              className="text-destructive text-[11px] truncate max-w-xs"
  ...
  if (job.processedOn) {
    ...
    if (endTs) {
      ...
      events.push({
        label: 'run',
        duration: processMs,
        color: status === 'failed' ? 'text-destructive' : 'text-blue-500',
      })
    }
    if (job.finishedOn) {
